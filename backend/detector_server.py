...
if GNN_AVAILABLE:
    try:
        ...
        if gnn_scorer.is_ready:
            ...
            # Fallback to Mock if native failed but ensemble provided one
            logger.warning("⚠️  GNN flow scorer not ready - checking ensemble mock")
    except Exception as e:
...
def api_test_cmd():
    ...
    try:
        ...
        if is_malicious:
            logger.warning(f"Malicious command detected: {severity} | {confidence:.1%} | {command[:50]}")
        else:
...
def classify_severity(score, is_anomaly, reason):
    ...
    if 'exfiltration' in reason_lower or 'massive' in reason_lower:
        return 'critical'
    if 'injection' in reason_lower or 'sql' in reason_lower:
        return 'high'
    if 'beacon' in reason_lower or 'c2' in reason_lower:
        return 'high'
    if 'portscan' in reason_lower:
    ...
    # Adjusted score thresholds for dynamic scoring model
    if score >= 0.8:
        return 'critical'
    if score >= 0.6:
        return 'high'
            ...
            try:
                ...
                if incoming_score is not None:
                    try:
                        ...
                        score = max(0.01, min(0.99, base + jitter))
                    except Exception:
                        pass
                ...
                try:
                    import random
                    if score > 0.95:
                         score = random.uniform(0.94, 0.99)
                    elif score < 0.05:
                         score = max(0.01, random.uniform(0.00, 0.03))
                    else:
                         score = max(0.01, min(0.99, score + random.uniform(-0.03, 0.03)))
                except:
                    pass

                if incoming_reason and ml_reason:
